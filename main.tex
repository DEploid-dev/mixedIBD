...
\begin{figure}[h]
  \begin{center}
  \includegraphics[width=\textwidth]{figures/Fig4-Classification.pdf}
   \caption{Identifying sibling strains within mixed infections.  (a) Generating IBD fraction and segment length distributions for unrelated ($M=0$, grey), sibling ($M=1$, purple), inbred-sibling ($M=2$, pink) and doubly-inbred-sibling ($M=3$, orange) mixed infections. Two clonal samples from a given country are combined to produce a $K=2$ unrelated mixed infection which recapitulates local latent IBD. This unrelated infection is then passed through $n$ rounds of `pf-meioses` a malaria-specific meioses simulator, to generate $M=n$ classes. (b) Simulated IBD distributions for $M=0...3$ for the African and Asian country with the most K=2 mixed infections. Simulated distributions are shown with circular points, black diamonds indicate statistics for real mixed infections discovered in the region (top) and their color at bottom shows their assignment after classification. (c) Classification result of 397 $k=2$ mixed infections from 14 countries. Unknown indicates mixed infections with IBD statistics that were never observed by simulation. (d) Breakdown of class frequency by country.} \label{fig:strainIBD}
   \end{center}
