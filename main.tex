...
\subsection{``Paint'' IBD posterior probabilities across the genome}
...
We use an IBD method to investigate the heterogeneity along the genome. Assuming independence along the genome, at every genomic position $i$, we use hyphen connected strain IDs to denote the IBD-state, indicating IBD-sharing between strains. For example, the state ``1-2-2'' implies that there are 3 strains, but only 2 distinctive strains, and strains 2 and 3 are IBD. Consequently, the possible haplotypes states of the given IBD-state ``1-2-2'' include $\{\{0,0,0\}, \{0,1,1\}, \{1,0,0\}, \{1,1,1\}\}$. For mixed of two strains, we only need to consider cases of IBD or non-IBD, with possible haplotype states of $\{\{0,0\}, \{1,1\}\}$ and $\{\{0,1\}, \{1,0\}\}$. For more than two strains, the IBD state space spans exponentially. For instance, three strains consist with 5 IBD states; four strains consist with 15 IBD states. For each IBD-state, number of haplotype states range from 2 to $2^{k}$. Therefore, this new method is not suitable for deconvolve complicated mixtures samples of five strains. From experience, we assume no more than five strains within one infection. Combining the haplotype state with strain proportions, we can compute the likelihood of data given the expected WSAF at each position. Assuming uniform prior on haplotype states given a IBD state, we can compute the posterior probabilities of IBD-state by accumulating the haplotype posterior probabilities.
