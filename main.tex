...
\todo{additional in silico lab mixture of 4s, \url{https://github.com/DEploid-dev/DEploid-Data-Benchmark-in_silico_lab_mixed_4s}}
...
\todo{Jason: change figure simulation figure, for k = 4, 4th strain color is not obvious, consider to use pink, need to check k = 3, the 1 bite case relatedeness}
...
\begin{figure}[htp]
  ...
  \figsupp{Simulation study panel. We conduct simulation studies to mimic $k$-mixtures (top row of the panel), of which $k$ takes value of 2, 3 and 4. For each $k$-mixture, the left column illustrate the overall relationship between strains: black dots indicate strains; connected dots imply strains come from the same infection. The level of relatedness between strains is reflected by the haplotype segment copied from their parents. For example, when $k=2$, we consider the case that the two strains are from two independent infections; on the other hand, when two strains are from the same mosquito bite, we consider low (25\%), moderate (50\%) and high (75\%) relatedness between two sibling strains which are represented in the second, third and forth rows respectively. For $k=3$, we consider mixed infections as products of three mosquito bites, two mosquito bites and a single bite. For $k=4$, we consider mixed infections as products of four mosquito bites, three mosquito bites, two mosquito bites (further divided to case when both bites pass on two sib strains and only one bite pass on sib strains) and a single bite.}
