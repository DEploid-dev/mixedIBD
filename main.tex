...
\begin{figure}[htp]
  ...
  \figsupp{Simulation study overview. We conduct simulation studies to mimic $k$-mixtures, when $k$ takes value of 2, 3 and 4. For each $k$-mixture, the left column illustrate the overall relationship between strains: black dots indicate strains. If two dots are connected by a black line, it implies these two strains are siblings. The level of relatedness between siblings is reflected by the haplotype segment copied from the parents. For example, when $k=2$, we consider the case that the two strains are from two independent infections; on the other hand, when two strains come from the same mosquito bite, we consider low (25\%), moderate (50\%) and high (75\%) relatedness between two sibling strains which are represented in the second, third and forth rows respectively. For $k=3$, we consider mixed infections as products of three mosquito bites, two mosquito bites and a single bite. For $k=4$, we consider mixed infections as products of four mosquito bites, three mosquito bites, two mosquito bites (further divided to case when both bites pass on two sib strains and only one bite pass on sib strains) and a single bite.}
