...
\donenum{Major Comment 7. on validation}

\subsubsection{Validation against experimentally generated mixed infections}
...
\begin{figure}[htp]
  ...
  \figsupp{Validation of {\tt DEploidIBD} using 27 {\it in vitro} lab mixtures and 4 {\it in silico} mixtures. A reference panel of the laboratory  strains (3D7, Dd2, HB3 and 7G8; Panel V) was used to deconvolve samples with {\tt DEploid}. Each experiment is performed with and without IBD inference and with the maximum number of 4 strain. Black crosses indicate the true effective number of strains. Coloured crosses ({\tt DEploid} in red, {\tt DEploidIBD} in purple) indicate median values obtained from 30 replicates using the algorithm indicated in the legend. The coloured dots show the inferred effective number of strains across replicates with intensity proportional to fraction. Note one sample where balanced proportions of three strains results in the LD-free ({\tt DEploid-IBD}) approach fitting the data as a mixture of two strains with proportions of $1\slash3$ and $2\slash3$. For {\it in silico } mixtures of four strains, {\tt DEploid} performs poorly. The {\tt DEploidIBD} new algorithm shows limited improvement in unbalanced mixtures, but completely misclassifies 4-mixtures as of 3s.}{\includegraphics[width=0.6\textwidth]{Fig2inVitro.pdf}}
  \label{figsupp:invitro}
  \donenum[inline]{Minor Comment 13, on figure supplement 2}

  \figsupp{Illustration of simulation study design. We conduct simulation studies to mimic $k$-mixtures (top row) as results of $m$-biting events, where $k \in \{2,3,4\}$ and $1 \leq m \leq k$. For each $k$-mixture, the left column illustrate the overall relationship between strains (black dots): connected dots imply strains are from the same mosquito bite. The level of relatedness between parasite strains is reflected by the haplotype segment copied from the parental strains within the mosquito. Each color represents a unique strain within the mosquito, which we randomly draw from field clonal haplotypes. For example, when $k=2$, we consider the case that the two strains are from two independent mosquito bites; on the other hand, when two strains are from the same mosquito bite, we consider scenarios of low (25\%), moderate (50\%) and high (75\%) relatedness between two sibling strains. These events are represented in the second, third and forth rows respectively. Note that 25\%, 50\% and 75\% are not actual events in real life, but rather than a choice for the simulation. For $k=3$, we consider mixed-infection events as products of three mosquito bites, two mosquito bites and a single bite. For $k=4$, we consider mixed infections as products of four mosquito bites, three mosquito bites, two mosquito bites and a single bite. We further divide the possibilities of the 2-bite event into the case that both bites pass on two strains (2+2) and the other possibility that one bite passes on a single strain and the other bit passes on three strains (1+3).}{\includegraphics[width = \textwidth]{Fig2SimScheme.pdf}}
  \label{figsupp:scheme}

  \figsupp{Additional comparison of \texttt{DEploidIBD} and \texttt{DEploid} on 100 {\it in silico} mixtures of two strains from Africa with low- and moderate- relatedness, illustrated by sub panels (A) and (B) respectively. Detailed panel description can be found in Figure~\ref{fig:benchmark} caption. {\tt DEploid} generally works well with samples of low within sample relatedness. It struggles when the minor strain proportion is below 30\%. Whereas {\tt DEploidIBD} overcomes this issue.}{\includegraphics[width = \textwidth]{Fig2Africa.pdf}}
  \label{figsupp:africa-low-moderate}

  \figsupp{Additional comparison of \texttt{DEploidIBD} and \texttt{DEploid} on {\it in silico} 2-bite mixtures of three strains from Africa and Asia, illustrated by sub panels (A) and (B) respectively. Detailed panel description can be found in Figure~\ref{fig:benchmark} caption. The unrelated strain provides a strong signal in allele frequency imbalance for {\tt DEploid} to detect. It therefore performs better than dealing with 1-bite mixtures. Comparing (A) and (B), pairwise relatedness becomes noisy in Asia because of the background IBD. On the other hand, background relatedness forms short segment of IBD tracts. It therefore shifts the distribution of the IBD tracts, which explains the IBD N50 underestimation in panel B.}{\includegraphics[width = \textwidth]{Fig2k3.pdf}}
  \label{figsupp:k3}


  \figsupp{Comparison of \texttt{DEploidIBD} and \texttt{DEploid} on 100 {\it in silico} 3-bite mixtures of four strains from Africa. Detailed panel description can be found in Figure~\ref{fig:benchmark} caption. {\tt DEploid} fails to recover any metrics in this experiment. Whereas {\tt DEploidIBD} works well when all four strains have unequal proportions, and works less effective when some strains have equal proportion.}{\includegraphics[width = \textwidth]{Fig2k4.pdf}}
  \label{figsupp:k4}
...
\hline
Gambia          &    2008 & Brikam   & 0.06   & 65     & 129 ( 9.4 )  & \textcolor{black}{0.5}\todonum[inline]{0.53} & 1.3 &\citet{Amambua-Ngwa2012}\\
\hline
Ghana           &    2009 & Navrongo & 0.79 & 121 & 86 ( 5.7 )& \textcolor{black}{0.21}\todonum[inline]{0.25} & 1.6&\multirow{3}{*}{\parbox{3.4cm}{\citet{Duffy2015,Kamau2015,eLife2016}}}\\
                & 2010 & Navrongo & 0.79 & 171 & 127 ( 10.3 )& \textcolor{black}{0.23}\todonum[inline]{0.26} & 1.5 &\\
                & 2011 & Navrongo & 0.72 & 97 & 76 ( 5.3 )& \textcolor{black}{0.21}\todonum[inline]{0.24} & 1.5& \\
                &      & Kintampo & 0.58 & 6 & 89 ( 13.5 )& \textcolor{black}{0.11}\todonum[inline]{0.16} & 1.5&\\
                & 2012 & Navrongo & 0.52 & 47 & 111 ( 3.8 )& \textcolor{black}{0.29}\todonum[inline]{0.31} & 1.6&\\
                &      & Kintampo & 0.41 & 40 & 157 ( 8.1 )& \textcolor{black}{0.22}\todonum[inline]{0.25} & 1.6&\\
                & 2013 & Navrongo & 0.31 & 88 & 119 ( 4 )& \textcolor{black}{0.26}\todonum[inline]{0.29} & 1.6&\\
                &      & Kintampo & 0.29 & 4 & 172 ( 38.4 )& \textcolor{black}{0.44}\todonum[inline]{0.53} & 1.1&\\
\hline
Malawi & 2011 & Chikwawa & 0.19 & 230 & 101 ( 3 )& \textcolor{black}{0.26}\todonum[inline]{0.28} & 1.7 &\citet{Ocholla2014}\\
       &      & Zomba & 0.34 & 35 & 89 ( 9.1 )& \textcolor{black}{0.24}\todonum[inline]{0.28} & 1.6 &\\
\hline
Mali & 2007 & Bandiagara & 0.43 & 9 & 95 ( 25.2 )& \textcolor{black}{0.39}\todonum[inline]{0.39} & 1.8  &\multirow{3}{*}{\parbox{3.4cm}{\citet{Mobegi2014,eLife2016}}}\\
     &      & Faladje & 0.37 & 36 & 75 ( 10.1 )& \textcolor{black}{0.27}\todonum[inline]{0.34} & 1.3 &\\
     &      & Kolle & 0.21 & 51 & 82 ( 10.5 )& \textcolor{black}{0.3}\todonum[inline]{0.36} & 1.6 &\\
\cline{1-8}
Guinea & 2011 & Nzerekore & 0.49 & 97 & 77 ( 4.6 )& \textcolor{black}{0.17}\todonum[inline]{0.21} & 1.4 &\\
\cline{1-8}
Congo DR        & 2013 & Kinshasa & 0.24 & 113 & 49 ( 3.2 )& \textcolor{black}{0.31}\todonum[inline]{0.36} & 1.5\\
\hline
Senegal & 2004 & Thies & 0.09 & 2 & 130 ( 68.2 )& \textcolor{black}{0.01}\todonum[inline]{0.03} & 1.4  &\citet{Wong2017}\\
        & 2009 & Thies & 0.04 & 43 & 175 ( 14.9 )& \textcolor{black}{0.43}\todonum[inline]{0.47} & 1.1  &\\
        & 2010 & Thies & 0.04 & 24 & 159 ( 9.7 )& \textcolor{black}{0.3}\todonum[inline]{0.36} & 1.3 &\\
        & 2011 & Thies & 0.03 & 32 & 97 ( 6 )& \textcolor{black}{0.33}\todonum[inline]{0.4} & 1.1 &\\
\hline
\hline
West     & 2009 & Pursat & 0.0071 & 19 & 75 ( 8.8 )& \textcolor{black}{0.39}\todonum[inline]{0.39} & 1.3 &\multirow{3}{*}{\parbox{3.4cm}{\citet{Amato2017, eLife2016}}}\\
Cambodia & 2010 & Pursat & 0.0071 & 105 & 95 ( 6.8 )& \textcolor{black}{0.65}\todonum[inline]{0.65} & 1.2&\\
         & 2011 & Pailin & 0.0025 & 49 & 54 ( 4.1 )& \textcolor{black}{0.43}\todonum[inline]{0.43} & 1.1&\\
         &      & Pursat & 0.0096 & 103 & 49 ( 3.1 )& \textcolor{black}{0.63}\todonum[inline]{0.63} & 1.2&\\
         & 2012 & Pailin & 0.00096 & 31 & 46 ( 5.6 )& \textcolor{black}{0.43}\todonum[inline]{0.43} & 1.0&\\
         &      & Pursat & 0.0079 & 7 & 37 ( 19.1 )& \textcolor{black}{0.58}\todonum[inline]{0.58} & 1.4&\\
\cline{1-8}
North & 2010 & Ratanakiri & 0.0039 & 50 & 71 ( 6.1 )& \textcolor{black}{0.43}\todonum[inline]{0.44} & 1.3&\\
Cambodia  & 2011 & Preah Vihear & 0.02 & 73 & 51 ( 5.3 )& \textcolor{black}{0.36}\todonum[inline]{0.36} & 1.2&\\
          &      & Ratanakiri & 0.0032 & 81 & 45 ( 4.3 )& \textcolor{black}{0.47}\todonum[inline]{0.48} & 1.4&\\
          & 2012 & Preah Vihear & 0.0075 & 30 & 43 ( 6.7 )& \textcolor{black}{0.37}\todonum[inline]{0.38} & 1.0&\\
          &      & Ratanakiri & 0.0016 & 15 & 44 ( 8.9 )& \textcolor{black}{0.3}\todonum[inline]{0.32} & 1.3&\\
\hline
Thailand & 2011 & Mae Sot & 0.00011 & 35 & 66 ( 7.5 )& \textcolor{black}{0.35}\todonum[inline]{0.35} & 1.2 &\multirow{3}{*}{\parbox{3.4cm}{\citet{Miotto2013, eLife2016}}}\\
         &      & Sisakhet & 1e-04 & 5 & 112 ( 25.4 )& \textcolor{black}{0.17}\todonum[inline]{0.17} & 1.3 &\\
         & 2012 & Mae Sot & 5.7e-05 & 69 & 83 ( 4.9 )& \textcolor{black}{0.58}\todonum[inline]{0.59} & 1.3 &\\
         &      & Ranong & 0.00018 & 11 & 82 ( 12.4 )& \textcolor{black}{0.38}\todonum[inline]{0.34} & 1.2 &\\
         &      & Sisakhet & 0 & 13 & 89 ( 13 )& \textcolor{black}{0.37}\todonum[inline]{0.37} & 1.1 &\\
         & 2013 & Sisakhet & 0 & 3 & 62 ( 8.8 )& \textcolor{black}{0.09}\todonum[inline]{0.09} & 1.2&\\
\cline{1-8}
Bangladesh & 2012 & Ramu & 0.0021 & 50 & 53 ( 4.2 )& \textcolor{black}{0.45}\todonum[inline]{0.49} & 1.5 &\\
\cline{1-8}
Viet Nam & 2011 & Bu Gia Map & 0.0073 & 43 & 67 ( 5 )& \textcolor{black}{0.43}\todonum[inline]{0.44} & 1.3&\\
         &      & Phuoc Long & 0.0053 & 27 & 68 ( 7.2 )& \textcolor{black}{0.37}\todonum[inline]{0.38} & 1.2&\\
         & 2012 & Bu Gia Map & 0.0072 & 19 & 115 ( 8 )& \textcolor{black}{0.67}\todonum[inline]{0.67} & 1.1&\\
         &      & Phuoc Long & 0.0048 & 5 & 107 ( 6.3 )& \textcolor{black}{0.81}\todonum[inline]{0.82} & 1.2&\\
\cline{1-8}
Myanmar & 2011 & Bago Division & 0.0076 & 12 & 59 ( 7.1 )& \textcolor{black}{0.24}\todonum[inline]{0.26} & 1.2 &\\
        & 2012 & Bago Division & 0.0084 & 47 & 62 ( 5.2 )& \textcolor{black}{0.45}\todonum[inline]{0.46} & 1.2 &\\
\cline{1-8}
Laos & 2011 & Attapeu & 0.0094 & 59 & 71 ( 4.2 )& \textcolor{black}{0.36}\todonum[inline]{0.37} & 1.4 &\\
     & 2012 & Attapeu & 0.02 & 25 & 77 ( 7.2 )& \textcolor{black}{0.68}\todonum[inline]{0.69} & 1.3 &\\
\hline
\hline
\end{tabular}
}
...
Given that samples from Senegal were screened to be primarily single-genome \citep{Daniels2015}, we computed all correlations with prevalence including ($r^{2}_{S+}$) and excluding them ($r^{2}_{S-}$), to isolate the possibility of sampling bias. We find that the effective number of strains is a significant predictor of $PfPR_{2-10}$ globally ($r^{2}_{S+} = 0.42$) and in African populations when Senegal is included ($r^{2}_{S+} = 0.23, p=0.04$), but is uncorrelated across Asia.  Similarly, $PfPR_{2-10}$ is negatively correlated with background IBD ($r^{2}_{S+} = 0.28, p=0.02$) across Africa but not in Asia.  Surprisingly, the amount of IBD observed within $K=2$ mixed infections was not correlated with prevalence in Africa or Asia. The rate of sibling infection ($M=1$) was also not correlated with the parasite prevalence ($r^{2}_{S+} = 0.03, p=0.5$). However, the rate of supersiblings ($K=2, M>1$) does exhibit a significant correlation with $PfPR_{2-10}$ ($r^{2}_{S+} = 0.10$) at the global scale, suggesting that serial co-transmission may occur more readily in low prevalence regions.



\begin{figure}[htp]
  \centering{}
  \includegraphics[width=\textwidth]{Fig7_mixed-prevalence_v4.pdf}
  \caption{The relationship between {\it P. falciparum} prevalence and characteristics of mixed infection. Four mixed infection statistics are shown including the average effective number of strains (Effective K, first column), given by $K_e = (\sum w_i^2)^{-1}$, where $w_i$ is the proportion of the $i$th strain; background IBD observed between clonal samples (Background Fraction IBD, second column); fraction IBD within $K=2$ mixed infections (Fraction IBD, third column); and the rate of $K=2$ mixed infections classified as having $M>1$ (Supersibling Rate, fourth column).  Each point relates to a row in Table~1 from different sampling locations and years. The variance explained was computed globally (shown at top for each statistic), across Asian countries (top panel) and across African countries (bottom panel) Where relevant, the variance explained was computed including Senegal ($r^{2}_{S+}$) and excluding Senegal ($r^{2}_{S-}$). (Significance levels: * $<0.05$, ** $<0.01$, *** $<0.001$)}
  \todonum[inline]{Minor Comment 2, on unlabelled points in Fig 6}
  \label{fig:model}
\end{figure}
