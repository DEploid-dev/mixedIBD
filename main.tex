...
Suppose there are $k$ parasite strains within one host, which implies that there exists $2^{k}$ number of genotypes at any given position along the genome. Consequently, this  forms $2^{k}$ allele frequency bands when combining the genotypes with their relative abundance. We refer these bands or patterns as within-sample allele frequency imbalance (WSAFI), which are visually obvious when $k$ is small (below 4 for instance) and strains are very distanced from each other (see Figure~\ref{fig:wsafi} left). Methods described in \citet{Jack2016} and \citet{Zhu2017} rely on WSAFI to separate the strains. However, when strains are related to each other, or strains proportions are similar to each other, these bands are less obvious, which makes deconvolution harder. By including genomic position information, WSAFI can be more easily identified along the genome. In this section, we will brief introduce the methods, but first let's introduce some notations.
