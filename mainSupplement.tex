...
\subsubsection{Adjust inbreeding prior}
...
\begin{itemize}
  \item {\bf Genotype states to allele states}
Note that the new method breaks the deconvolution process into two parts: 1. Inferring the proportion and IBD state. 2. Use a reference panel to tune the haplotypes (see \cite{Zhu2017} for details), once we fix the proportions. This implementation detail is only relating to the first part, where the allelic state is used to reflect as part of the IBD state. Therefore, we make $P(\mathbf{h} | \xi) = 1$, and strictly copy the underline genotypes (from the IBD state) as the allelic state. The impact of sequencing error on the expected WSAF is adjusted using equation \eqref{eqn:adj_q}.
