...
In the new method, {\tt DEploidIBD} does not reinforce any haplotype structure. Haplotype structures are independent between sites. However, the IBD-states are connected. At every site, the posterior probability maximizes at IBD state. We use a forward process to compute the posterior probabilities for IBD-states, and then sample backwards to obtain IBD-states and underline genotype states. For simplicity, we make $P(\mathbf{h} | \xi) = 1$, and strictly copy the underline genotypes (from the IBD state) as the allelic state. The impact of sequencing error on the expected WSAF is adjusted using equation \eqref{eqn:adj_q}.
