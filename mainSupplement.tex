...
\item {\bf Extract pairwise IBD tracks and relatedness}
\donenum{pairwise IBD tracks and relatedness}
At each site, we use the viterbi algorithm and  extract IBD states with the maximal forward posterior probabilities to obtain IBD state blocks. For each strain pair, we then identify pairwise IBD tracks from IBD state blocks which contain the target pairs. For instance, for 3 strains, we identify 0-1 tracks from blocks of IBD state 1-1-2 which only strains 0 and 1 are IBD and from IBD state 2-2-2 that all strains are IBD. IBD track lengths are computed by taking positional differences at the two ends of IBD tracks.
...
\section{Deconvolution of {\it in silico} field mixtures}
