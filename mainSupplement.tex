...
At each site, we use the viterbi algorithm and  extract IBD states with the maximal forward posterior probabilities to obtain IBD state blocks. For each strain pair, we identify pairwise IBD tracks from IBD state blocks. For instance, for 3 strains, we identify 0-1 tracks from blocks of 1-1-2 and 2-2-2. IBD track lengths are computed by taking positional differences at the two ends of IBD tracks.

Genomic data are usually heterogeneous: Both of the read coverage and depth vary along the sequence. In order to obtain a smooth posterior probabilities along the sequence, we take into account of transitions from both sides of a SNP. Once we determine the proportion of each strain. We fix the proportion, and use a forward-backward algorithm to compute the posterior probabilities (unlike the forward posterior during the MCMC steps). Finally, we integrate the posterior probabilities for each strain pair, and obtain the pairwise relatedness by taking the average among all possible pairs.
\end{itemize}
